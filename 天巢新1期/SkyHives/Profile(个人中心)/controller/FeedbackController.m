...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    //    *键盘弹起
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyborardDidShow:) name:UIKeyboardWillShowNotification  object:nil];
    
    //    /**键盘落下*/
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardDidHidden) name:UIKeyboardWillHideNotification object:nil];
...
-(void)keyborardDidShow:(NSNotification *)Info{
    ...
    if (self.textView.isFirstResponder) {
        ...
        if (textViewY<keyboardY) {
            ...
            self.view.y -= (textViewY-keyboardY);
        }
    }
    if(self.PhoneTextField.isFirstResponder)
    {
        ...
        if (textFiledY<keyboardY) {
            ...
            self.view.frame = CGRectMake(0, 0, ApplicationframeValue.width, ApplicationframeValue.height);
            self.view.y -= (textFiledY-keyboardY);
        }
