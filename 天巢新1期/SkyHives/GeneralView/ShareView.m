...
{
    float Margin = ceilf(frame.size.width/self.imageArr.count/2);
    ...
    if (self) {
        ...
        UIView *viewS = [[UIView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
        viewS.backgroundColor =[UIColor whiteColor];
        [self addSubview:viewS];
        
        UILabel *TitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.width, 55)];
...
//        [viewS addSubview:lineImage];
        ...
        for (int i = 0; i < [self.imageArr count]; i++) {
            ...
            btn.tag = i +1;
            btn.frame = CGRectMake(Margin/2+2*Margin*i,CGRectGetMaxY(TitleLabel.frame)+25, Margin, Margin);
